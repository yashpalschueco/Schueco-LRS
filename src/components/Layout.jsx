  ...
  const sidebar = (
    <>
      ...
      <div className="px-5 py-4 border-t" style={{ borderColor: '#1E1E1E' }}>
        ...
        <button onClick={() => supabase.auth.signOut()} className="text-xs transition-colors hover:text-white" style={{ color: '#5A5450' }}>
          Sign out →
        </button>
      </div>
    </>
  ...
  return (
    <div className="flex h-screen overflow-hidden font-sans">
      ...
      <div className="fixed top-0 left-0 right-0 z-40 flex md:hidden items-center justify-between px-4 h-14" style={{ background: '#0F0F0F' }}>
        ...
        <button onClick={() => setDrawerOpen(!drawerOpen)} className="text-white w-8 h-8 flex items-center justify-center rounded-lg"
      ...
      {drawerOpen && (
        <div className="fixed inset-0 z-30 md:hidden" onClick={() => setDrawerOpen(false)}>
          ...
          <div className="absolute top-14 right-0 bottom-0 w-64 flex flex-col overflow-y-auto" style={{ background: '#0F0F0F' }} onClick={e => e.stopPropagation()}>
            {sidebar}
          </div>
        </div>
      )}
