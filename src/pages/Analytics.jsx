...
export default function Analytics() {
  ...
  const monthMap = {}
  inquiries.forEach(i => { if (i.created_at) { const k = i.created_at.slice(0,7); monthMap[k] = (monthMap[k]||0)+1 } })
  const monthlyStats = Object.entries(monthMap).sort(([a],[b]) => a.localeCompare(b)).slice(-12)
    .map(([key, count]) => ({ label: new Date(key+'-01').toLocaleDateString('en-IN',{month:'short',year:'2-digit'}), count }))
  ...
  // ── Workload thresholds ──────────────────────────────────────────────────
  const fabAvgActive  = fabStats.length ? fabStats.reduce((a,f)=>a+f.active,0)/fabStats.length : 0
  const archAvgActive = archStats.length ? archStats.reduce((a,f)=>a+f.active,0)/archStats.length : 0

  function loadFlag(active, avg) {
    if (avg === 0) return { label: 'No data', bg: '#F3F4F6', color: '#6B7280' }
    if (active >= avg * 1.5) return { label: 'Overloaded', bg: '#FEF2F2', color: '#B91C1C' }
    if (active <= avg * 0.5) return { label: 'Has capacity', bg: '#ECFDF5', color: '#065F46' }
    return { label: 'Balanced', bg: '#FFFBEB', color: '#92400E' }
  }

  // ── Stale inquiries (15+ days, still New/Quoted) ─────────────────────────
  const staleInquiries = inquiries
    .filter(i => (i.status === 'New' || i.status === 'Quoted') && daysSince(i.created_at) >= 15)
    .map(i => ({ ...i, days: daysSince(i.created_at) }))
    ...
    <div className="p-8 max-w-6xl">
      ...
      {tab === 'overview' && (
        <>
          ...
          <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100"><span className="font-medium text-gray-900 text-sm">Monthly Trend — New Inquiries (last 12 months)</span></div>
            <div className="p-5">
              {monthlyStats.length === 0 ? <p className="text-sm text-gray-400 text-center py-6">No data</p> : (
                <div className="flex items-end gap-2" style={{ height: 120 }}>
                  {monthlyStats.map(m => (
                    <div key={m.label} className="flex-1 flex flex-col items-center gap-1 min-w-0">
                      <div className="text-[10px] text-gray-500 font-medium">{m.count}</div>
                      <div className="w-full rounded-t-sm" style={{ height: `${Math.max(4,(m.count/maxMonth)*88)}px`, background: '#C9A44A', opacity: 0.85 }} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100">
              ...
              <p className="text-[11px] text-gray-400 mt-0.5">Based on currently active (New + Quoted) inquiries</p>
            </div>
            <div className="divide-y divide-gray-50 max-h-[36rem] overflow-y-auto">
              {[...fabStats].sort((a,b) => b.active - a.active).map(f => {
                const flag = loadFlag(f.active, fabAvgActive)
                return (
                  <div key={f.name} className="px-5 py-3 flex items-center justify-between">
                    <div className="min-w-0"><div className="text-sm font-medium text-gray-800 truncate">{f.name}</div><div className="text-[11px] text-gray-400">{f.active} active inquiries</div></div>
                    <Pill bg={flag.bg} color={flag.color}>{flag.label}</Pill>
                  </div>
                )
              })}
              {fabStats.length === 0 && <p className="text-sm text-gray-400 text-center py-8">No data yet</p>}
            </div>
          </div>
          <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100">
              <span className="font-medium text-gray-900 text-sm">Architect Workload</span>
              <p className="text-[11px] text-gray-400 mt-0.5">Based on currently active (New + Quoted) inquiries</p>
            </div>
            <div className="divide-y divide-gray-50 max-h-[36rem] overflow-y-auto">
              {[...archStats].sort((a,b) => b.active - a.active).map(a => {
                const flag = loadFlag(a.active, archAvgActive)
                return (
                  <div key={a.name} className="px-5 py-3 flex items-center justify-between">
                    <div className="min-w-0"><div className="text-sm font-medium text-gray-800 truncate">{a.name}</div><div className="text-[11px] text-gray-400">{a.active} active inquiries</div></div>
                    <Pill bg={flag.bg} color={flag.color}>{flag.label}</Pill>
                  </div>
                )
              })}
              {archStats.length === 0 && <p className="text-sm text-gray-400 text-center py-8">No data yet</p>}
            </div>
          </div>
        </div>
      )}
      ...
      {tab === 'stale' && (
        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100">
            ...
            <p className="text-[11px] text-gray-400 mt-0.5">Still New or Quoted after 15+ days — nothing should be silently forgotten</p>
          </div>
          <div className="divide-y divide-gray-50 max-h-[36rem] overflow-y-auto">
