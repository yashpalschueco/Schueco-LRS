    ...
    <div className="p-8 max-w-6xl">
      ...
      {tab === 'overview' && (
        <>
          ...
          {/* Monthly Sales by Region — stacked by value, hover shows count too */}
          <div className="bg-white border border-gray-200 rounded-xl overflow-hidden mt-6">
            <div className="px-5 py-4 border-b border-gray-100">
              <span className="font-medium text-gray-900 text-sm">Monthly Sales by Region (last 12 months)</span>
              <p className="text-[11px] text-gray-400 mt-0.5">Bar height = pipeline value · hover a segment to see value and inquiry count for that region</p>
            </div>
            <div className="p-5">
                ...
                <>
                  <div className="flex items-end gap-2" style={{ height: 140 }}>
                    {monthlyRegionStats.map(m => (
                      <div key={m.label} className="flex-1 flex flex-col items-center gap-1 min-w-0">
                        <div className="text-[10px] text-gray-500 font-medium">{m.totalValue > 0 ? fmtCr(m.totalValue) : ''}</div>
                        <div className="w-full rounded-t-sm overflow-hidden flex flex-col-reverse" style={{ height: 100 }}>
                          {REGION_LIST.map(r => {
                            const seg = m.byRegion[r]
                            if (!seg || seg.value <= 0) return null
                            const segHeight = Math.max(2, (seg.value / maxMonthRegionValue) * 100)
                            return (
                              <div
                                key={r}
                                title={`${r}: ${fmtCr(seg.value)} · ${seg.count} ${seg.count === 1 ? 'inquiry' : 'inquiries'}`}
                                style={{ height: `${segHeight}px`, background: REGION_COLORS[r], opacity: 0.88 }}
