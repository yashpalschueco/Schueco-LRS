...
export default function EditInquiry() {
  ...
  useEffect(() => {
    async function load() {
      ...
      // Store original values for change tracking in email notification
      originalRef.current = {
        clientName:       inq.client_name          || '',
        projectName:      inq.project_name          || '',
        status:           inq.status               || 'Ongoing',
        projectValue:     String(inq.project_value || ''),
        siteLocation:     inq.site_location         || '',
        region:           inq.region               || '',
        source:           inq.source               || '',
        meetingWithClient:inq.meeting_with_client   || '',
  ...
  async function handleSubmit(e) {
    ...
    const newVals = {
      ...
      status: form.status, projectValue: String(form.projectValue || ''),
      siteLocation: form.siteLocation.trim(), region: form.region || '',
      source: form.source || '', meetingWithClient: form.meetingWithClient || '',
      legacyNew: form.legacyNew || '', productsOffered: form.productsOffered || '',
      cpsNotes: form.cpsNotes.trim(), quoteApproved: form.quoteApproved || '',
      boqReceived: form.boqReceived || '', beMonthBooking: form.beMonthBooking || '',
    ...
    // Sync to OneDrive + trigger email notification — non-blocking
    fetch('/api/sync-onedrive', {
      ...
      body: JSON.stringify({
        ...
        inquiry: {
          ...
          notes:               form.notes.trim() || '',
          partner2_name:       form.partner2Id ? getName(fabricators, form.partner2Id) : '',
          partner3_name:       form.partner3Id ? getName(fabricators, form.partner3Id) : '',
        }
      })
