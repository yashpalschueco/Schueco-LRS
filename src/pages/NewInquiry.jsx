...
import { findStrongDuplicates, findWeakDuplicates } from '../utils/fuzzy'
...
export default function NewInquiry() {
  ...
  const [duplicate,       setDuplicate]       = useState(null)   // exact match
  const [strongMatches,   setStrongMatches]   = useState([])     // tier 1 block
  const [weakMatches,     setWeakMatches]     = useState([])     // tier 2 warn
  ...
  useEffect(() => {
    Promise.all([
      ...
      supabase.from('inquiries').select('id,client_name,project_name,status,architect_id,schueco_person_id,fabricator_id,created_by_email,created_at'),
    ]).then(([a, f, t, inq]) => {
  ...
  function set(key, val) {
    setForm(f => ({ ...f, [key]: val }))
    // Clear warnings when user edits relevant fields
    if (['clientName', 'projectName', 'architectId'].includes(key)) {
      setStrongMatches([])
      setWeakMatches([])
      setDuplicate(null)
    }
  ...
  async function handleSubmit(e, opts = {}) {
    ...
    // ── Step 2: Strong fuzzy — client + project + same architect ──────────────
    if (!opts.skipStrong) {
      const strong = findStrongDuplicates(clientName, projectName, architectId, allInquiries)
      if (strong.length > 0) {
    ...
    // ── Step 3: Weak fuzzy — client + project similar, lower confidence ───────
    if (!opts.skipWeak) {
      const weak = findWeakDuplicates(clientName, projectName, architectId, allInquiries)
      if (weak.length > 0) {
        setWeakMatches(weak)
        setSaving(false)
        scrollToWarning()
    ...
    // ── Step 4: Save ──────────────────────────────────────────────────────────
    // serial_no is NOT set manually here — the database assigns it
    // automatically via a sequence, so it's always correct even if
    // multiple people save inquiries at the exact same moment.
    ...
    const { data: inserted, error } = await supabase.from('inquiries').insert({
      ...
      responsible_name:          getName(team, schuecoPersonId)        || null,
      fabricator_name:           getName(fabricators, fabricatorId)    || null,
      architect_name:            getName(architects, architectId)      || null,
    ...
    // Sync to OneDrive Excel — non-blocking, won't affect user if it fails
    const savedId = form.clientName.trim() // placeholder, actual ID is set above
  ...
  return (
    <div className="p-8 max-w-lg">
      ...
      <div ref={warningRef}>
        ...
        {/* Strong fuzzy — ORANGE, hard stop */}
        {/* Client similar + Project similar + Same architect = very likely duplicate */}
        {strongMatches.length > 0 && !duplicate && (
          <div className="mb-5 border border-orange-300 rounded-xl overflow-hidden">
            <div className="bg-orange-50 px-5 py-4 border-b border-orange-200">
              <p className="text-sm font-semibold text-orange-800">⚠ This appears to be a duplicate</p>
              <p className="text-xs text-orange-700 mt-1">
                Similar client name + similar project + same architect found. Please verify before proceeding.
              </p>
            </div>
            <div className="px-5 py-3 bg-orange-50/40">
              {strongMatches.map(m => (
                <div key={m.id} className="py-2 border-b border-orange-100 last:border-0 text-xs">
                  ...
                  <div className="text-gray-400 mt-0.5">Registered by {(m.created_by_email || '').split('@')[0] || '—'} · {fmt(m.created_at)}</div>
                </div>
              ))}
            </div>
            <div className="px-5 py-3 bg-orange-50/40 flex gap-3 flex-wrap">
              <button
                onClick={() => setStrongMatches([])}
                className="px-4 py-2 text-sm font-medium text-orange-800 bg-orange-100 rounded-lg hover:bg-orange-200 transition-colors"
              >
                Go back and review
              </button>
              <button
                onClick={() => handleSubmit(null, { skipStrong: true })}
                className="px-4 py-2 text-sm font-medium text-white rounded-lg hover:opacity-85 transition-opacity"
                style={{ background: '#0F0F0F' }}
              >
                I confirm this is a new inquiry
              </button>
            </div>
          </div>
        )}

        {/* Weak fuzzy — YELLOW, warn + allow */}
        {/* Client + Project similar but lower confidence (possible spelling mistake) */}
        {weakMatches.length > 0 && !duplicate && strongMatches.length === 0 && (
          <div className="mb-5 border border-yellow-200 rounded-xl overflow-hidden">
            <div className="bg-yellow-50 px-5 py-4 border-b border-yellow-200">
              <p className="text-sm font-semibold text-yellow-800">⚠ Similar client and project found</p>
              <p className="text-xs text-yellow-700 mt-1">
                Could be a spelling variation. Please verify this is genuinely a new inquiry.
              </p>
            </div>
            <div className="px-5 py-3 bg-yellow-50/30">
              {weakMatches.map(m => (
                <div key={m.id} className="py-2 border-b border-yellow-100 last:border-0 text-xs">
                  <div className="flex gap-2 flex-wrap">
                    <span className="font-medium text-gray-800">{m.client_name}</span>
                    <span className="text-gray-300">·</span>
                    <span className="text-gray-600">{m.project_name}</span>
            ...
            <div className="px-5 py-3 bg-yellow-50/30 flex gap-3 flex-wrap">
              <button
                onClick={() => setWeakMatches([])}
                className="px-4 py-2 text-sm text-yellow-800 bg-yellow-100 rounded-lg hover:bg-yellow-200 transition-colors"
              >
                Go back and check
              </button>
              <button
                onClick={() => handleSubmit(null, { skipStrong: true, skipWeak: true })}
                className="px-4 py-2 text-sm font-medium text-white rounded-lg hover:opacity-85 transition-opacity"
                style={{ background: '#0F0F0F' }}
              >
                Yes, this is a new inquiry
              </button>
            </div>
          </div>
        )}
