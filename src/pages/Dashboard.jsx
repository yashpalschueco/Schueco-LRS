  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
      <div className="p-4" onClick={onToggle}>
        ...
        {/* Client + project */}
        <div className="font-medium text-gray-900 text-sm">{inq.client_name}</div>
        <div className="text-xs text-gray-500 mt-0.5 truncate">{inq.project_name}</div>
        {inq.site_location && <div className="text-[11px] text-gray-400 mt-0.5">{inq.site_location}</div>}

        {/* Owners row */}
        <div className="flex flex-wrap gap-x-3 gap-y-1 mt-2.5 text-[11px] text-gray-500">
          <span>{getName(team, inq.schueco_person_id)}</span>
          <span className="text-gray-300">·</span>
          <span>{getName(fabricators, inq.fabricator_id)}</span>
          <span className="text-gray-300">·</span>
          <span>{getName(architects, inq.architect_id)}</span>
        </div>

        {/* Value + date + actions */}
        <div className="flex items-center justify-between mt-3 pt-2.5 border-t border-gray-100">
          <div className="flex items-center gap-3">
            <span className="text-sm font-medium" style={{ color: inq.project_value ? '#065F46' : '#D1D5DB' }}>
              {inq.project_value ? `₹${fmtCr(parseFloat(inq.project_value))} Cr` : '—'}
            </span>
      ...
      fetch('/api/sync-onedrive', {
        ...
        body: JSON.stringify({
          ...
          inquiry: {
            ...
            region:              inq.region              || '',
            site_location:       inq.site_location       || '',
            cps_notes:           inq.cps_notes           || '',
            source:              inq.source              || '',
            products_offered:    inq.products_offered    || '',
            meeting_with_client: inq.meeting_with_client || '',
            legacy_new:          inq.legacy_new          || '',
            boq_received:        inq.boq_received        || '',
            quote_approved:      inq.quote_approved      || '',
            be_month_booking:    inq.be_month_booking    || '',
            material_delivered:  inq.material_delivered  || '',
            be_month_invoicing:  inq.be_month_invoicing  || '',
            notes:               inq.notes               || '',
            partner2_name:       inq.partner2_name       || '',
            partner3_name:       inq.partner3_name       || '',
          }
        })
  ...
  return (
    <div className="p-4 sm:p-8">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4 mb-6">
        <div>
          ...
          <div className="flex items-center gap-2 sm:gap-3 mt-1.5 flex-wrap">
            ...
            <div className="flex items-center gap-1.5 bg-white border border-gray-200 rounded-lg px-2.5 sm:px-3 py-1">
            ...
            <div className="flex items-center gap-1.5 bg-emerald-50 border border-emerald-100 rounded-lg px-2.5 sm:px-3 py-1">
            ...
            {showFilteredVal && filteredPipeline !== totalPipeline && (
              <div className="flex items-center gap-1.5 rounded-lg px-2.5 sm:px-3 py-1" style={{ background: 'rgba(201,164,74,0.1)', border: '1px solid rgba(201,164,74,0.3)' }}>
        ...
        <button onClick={() => navigate('/new')} className="px-4 py-2 text-sm font-medium text-white rounded-lg hover:opacity-85 transition-opacity flex-shrink-0 w-full sm:w-auto text-center" style={{ background: '#0F0F0F' }}>
          + New Inquiry
        </button>
      </div>

      {/* ── Status cards ── */}
      <div className="grid grid-cols-2 sm:flex gap-3 mb-5">
        {stats.map(s => (
          <div key={s.label} onClick={() => setStatusFilter(prev => prev === s.label ? 'All' : s.label)}
            className="bg-white border border-gray-200 rounded-lg px-4 py-3 cursor-pointer transition-all"
      ...
      <input value={search} onChange={e => setSearch(e.target.value)}
        placeholder="Search client, architect, fabricator, region..."
          ...
          <div className="md:hidden space-y-3">
            ...
            {filtered.length > 1 && (
              <div className="text-center py-3 text-sm text-gray-500">
                {filtered.length} inquiries · ₹{fmtCr(filteredPipeline)} Cr pipeline
              </div>
            )}
          </div>
