...
//   _sameArchitect: true if the matched entry has the same architect
//
// Threshold: 60% — catches typos and partial names.
// If the architect also matches exactly, this is flagged as a stronger signal.
export function findClientMatches(clientName, inquiries, excludeId = null, threshold = 0.60, architectId = null) {
  ...
  for (const i of inquiries) {
    ...
    // Same architect = much stronger duplicate signal
    const sameArchitect = architectId && i.architect_id === architectId

    results.push({ ...i, _matchedVia: matchedVia, _score: score, _sameArchitect: sameArchitect })
  }

  return results
    .sort((a, b) => {
      // Sort same-architect matches first, then by score
      if (a._sameArchitect && !b._sameArchitect) return -1
      if (!a._sameArchitect && b._sameArchitect) return 1
